-- Fixed migration script to add new columns to existing tables
-- This script prevents infinite recursion in the trigger function
...
BEGIN
    ...
    -- Add last_generated_date column
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'tasks' AND column_name = 'last_generated_date') THEN
        ALTER TABLE tasks ADD COLUMN last_generated_date DATE;
    END IF;
...
-- Create function to generate recurring task instances (FIXED VERSION)
...
BEGIN
  ...
  IF NEW.is_recurring = TRUE THEN
    current_date_val := COALESCE(NEW.last_generated_date, NEW.recurrence_start_date);
    ...
    -- Update last_generated_date using a separate function to avoid recursion
    PERFORM update_task_last_generated_date(NEW.id, current_date_val);
  END IF;
...
-- Create a separate function to update last_generated_date without triggering recursion
CREATE OR REPLACE FUNCTION update_task_last_generated_date(task_id UUID, new_date DATE)
RETURNS void AS $$
BEGIN
  -- Use a direct update without triggering the main trigger
  UPDATE tasks 
  SET last_generated_date = new_date
  WHERE id = task_id;
END;
$$ language 'plpgsql';

-- Create trigger to generate recurring instances if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'generate_recurring_task_instances') THEN
...
SET 
  ...
  last_generated_date = COALESCE(last_generated_date, due_date)
